...
async function recalculateKPIs(): Promise<void> {
  ...
  await supabase.from('dashboard_kpis').upsert({
    id: 1, total_revenue: totalRevenue, best_month: best.month,
    best_month_value: Number(best.revenue), monthly_average: monthlyAverage,
    total_payouts: totalPayouts, yoy_growth: 0,
  })
}

...
export async function POST(request: NextRequest) {
  ...
  if (body.monthly_revenue) {
    ...
    if (replace) await ( supabaseAdmin as any).from('monthly_revenue').delete().neq('id', 0)
  ...
  if (body.client_data) {
    ...
    if (replace) await ( supabaseAdmin as any).from('client_data').delete().neq('id', 0)
  ...
  if (body.operator_payouts) {
    ...
    if (replace) await ( supabaseAdmin as any).from('operator_payouts').delete().neq('id', 0)

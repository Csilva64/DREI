...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (contentType.includes('text/csv')) {
    ...
    if (typeof body !== 'string') return res.status(400).json({ error: 'Expected raw CSV text body' })
  ...
  if (!rawRows.length) return res.status(400).json({ error: 'No rows found in payload' })
  ...
  return res.status(200).json({ ok: true, inserted: rows.length, message: `${table} updated and KPIs recalculated` })
}
